...
public class FileInfoDAO {
    ...
    private static final String SELECT_BY_OFN_QUERY = "SELECT * FROM avatars WHERE original_file_name = ?";
    ...
    public FileInfo getByIdUser(Integer id_user) throws SQLException {
        ...
        if (resultSet.next()) {
            ...
            fileInfo = new FileInfo(id, originalFileName, storageFileName, size, type, id_user);
        }
        return fileInfo;
    }
    ...
    /*public FileInfo getByOFN(String originalFileName) throws SQLException {
        FileInfo fileInfo = null;

        Connection connection = dataSource.getConnection();
        PreparedStatement preparedStatement = connection.prepareStatement(SELECT_BY_OFN_QUERY);

        preparedStatement.setString(1, originalFileName);
        ResultSet resultSet = preparedStatement.executeQuery();
