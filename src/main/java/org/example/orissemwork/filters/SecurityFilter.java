...
public class SecurityFilter extends HttpFilter {
    protected final String[] protectedPaths = {"/profile", "/settings", "/my_questions", "/all_questions",
            "/question", "/favorites_answers", "/admin_menu", "/all_users", "/all_categories"};

    protected final String[] adminPaths = {"/admin_menu", "/all_users", "/all_categories"};
    ...
    public void init(FilterConfig filterConfig) throws ServletException {
        ...
        categoryDAO = (CategoryDAO) filterConfig.getServletContext().getAttribute("categoryDAO");
    }
    ...
    protected void doFilter(HttpServletRequest req, HttpServletResponse res, FilterChain chain) {
        ...
        if (prot && !UserService.isSigned(req)) {
            ...
            if (UserService.isSigned(req)) {
                ...
                if ("/all_questions".equals(req.getRequestURI())) {
                    if (!req.getParameterMap().isEmpty()) {
                        String potencialCategory = req.getParameterMap().get("category")[0];
                        if (categoryDAO.getByName(potencialCategory) == null) {
                            res.sendError(HttpServletResponse.SC_NOT_FOUND, "Page not found");
                            return;
                        }
                    }
